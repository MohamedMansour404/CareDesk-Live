...
export class AnalyticsService {
  ...
  constructor(
    ...
    if (redisUrl) {
      this.redis = createRedisClient(redisUrl, { maxRetriesPerRequest: 3 });
      this.redis.on('error', (err) => {
        this.logger.debug(`Analytics Redis error: ${err.message}`);
      });
      this.logger.log('Analytics Redis cache initialized');
    }
  }
